...
router.get("/searchname", (req, res) => {
  ...
  myquery =
    "SELECT * FROM mydb.inventory_part WHERE LOWER(name) LIKE LOWER('%" +
    name +
    "%')";
  pool.query(myquery, function (err, rows, fields) {
    if (err) throw err;
...
router.post("/buy", (req, res) => {
  ...
  (async function sendquery(param) {
    ...
    const results = await Promise.all(queries).catch(() => {
      console.log("One of the queries failed to complete.");
    });
    ...
    results.forEach(([rows, fields]) => {
      valid.push(rows[0]);
      if (rows[0].cost_matches == 0 || rows[0].enough_stock == 0) status = 412;
    });
    ...
    //else transaction is valid, so insert the transaction row and update the part quantities
      ...
      const results = await Promise.all(queries).catch(() => {
        console.log("One of the queries failed to complete 2.");
      });
...
router.post("/rent", (req, res) => {
  ...
  (async function sendquery(param) {
    ...
    if (status != 200) {
      ...
      results.forEach(([rows, fields]) => {
        if (rows.length != 0) {
          console.log("That student has a hold");
          status = 412;
        }
      });
      results.forEach(([rows, fields]) => {
...
router.post("/return", (req, res) => {
  ...
  (async function sendquery(param) {
    ...
    results.forEach(([rows, fields]) => {
      if (rows.length == 0) {
        console.log("Nobody has that part checked out at this time");
        status = 412;
      }
    });
    results.forEach(([rows, fields]) => {
      valid.push(rows[0]);
      console.log(rows[0].net_id);
    });
    results.forEach(([rows, fields]) => {
      studentRenting = rows[0].net_id;
    });
    console.log("VALID: " + valid);
    ...
    //else transaction is valid, so insert the transaction row for the tool
      ...
      //for (i = 0; i < req.body.cart.length; i++) {
      var query = toUnnamed(
        "UPDATE mydb.rented_tool SET returned_date = NOW(3) WHERE tool_id = :id AND returned_date is NULL;" +
